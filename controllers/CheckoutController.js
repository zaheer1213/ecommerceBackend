...
exports.createCheckout = asyncHandler(async (req, res) => {
    try {
        const { userId, address, shippingMethod, paymentMethod, email, phone, status } = req.body;
...
`
        ...
        res.status(201).json({ message: 'Checkout successful without payment', checkout: newCheckout });
    } catch (error) {
        console.error(error);
...
exports.getUserCheckout = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to get checkout details', details: error.message });
    }
});
...
exports.updateCheckoutStatus = asyncHandler(async (req, res) => {
    try {
        ...
        const checkout = await Checkout.findByIdAndUpdate(checkoutId, { status }, { new: true });
...
exports.deleteCheckout = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to delete checkout', details: error.message });
    }
}); 
